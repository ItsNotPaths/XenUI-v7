...
def load_assets():
    ...
    configEditorBGtxt = BannerFont.render('Press Enter to save' + str([' ' * 73])[2:-2] + 'Press Escape to discard changes', False, (250, 250, 250))
    ...
    BuildPathsSurface.fill('#878236')
    BuildPathsSurface.set_alpha(100)
...
def askforgamepath():
    ...
    while Valid == False:
        ...
        screen.blit(BannerFont.render('No games file was found, please press the paste button below to paste a valid games file location to use', False, (250, 250, 250)), (140, 460))
        screen.blit(BannerFont.render(text, False, (250, 250, 250)), (240, 580))
        screen.blit(BannerFont.render(popup, False, (250, 250, 250)), (900, y))
        screen.blit(PasteSurface[0], (PasteSurface[1], PasteSurface[2]))
...
while run: # this is the main loop. father forgive me for i have sinned
    ...
    if timer > 80:
        timer = 0
        BuildPathsText = ''
    elif pygame.time.get_ticks() > 3300:
        intro_objects = []
            ...
            if mousestate == 1 and configEditorOpen == False and SettingsOpen == False:
                ...
                for i2, v2 in enumerate(games): # you need to copy the config file to the specified xenia build path if there is one, instead of the root folder
                    if v2[0] == GameButtons[i][5]:
                        ...
                        shutil.copyfile(Root + '\\' + 'Games' + '\\' + v2[0] + '\\' + games[i2][3], exepath[:exepath.rfind('\\')] + '\\' + games[i2][3])
                        try:
