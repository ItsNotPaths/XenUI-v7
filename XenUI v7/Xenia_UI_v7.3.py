...
def get_mouse():
    ...
    NewRange = 960
    mx = (((pygame.mouse.get_pos()[0]) * 960) / WindowInfo[0])
    my = (((pygame.mouse.get_pos()[1]) * 540) / WindowInfo[1])
...
def load_assets():
    global ui_assets, testsurface, banners, Root, imgW, imgH, ratio, BannerFont, banner_height, SubmenuBGSurface, configEditorBGtxt, ImportantConfigNames, ImportantConfigs, tooltips, TooltipSurfaceBack, ConfigFont, TooltipFont, ConfigTextSurface, SettingsTips, Root, UIRoot
    ...
    SubmenuBGSurface = pygame.Surface((screen_width - 50, screen_height - 60)); SubmenuBGSurface.fill('black')
...
def render():
    ...
    if SettingsOpen:
        ...
        for i, v in enumerate(SettingsTipsText):
            screen.blit(TooltipFont.render(str(v), False, (250, 250, 250)), (550, TooltipSurfaceBack[2] + (i * 22) + 8))
    pygame.draw.rect(screen, 'black', pygame.Rect(13, SubmenuBGSurfaceXvar, screen_width - 24, 500),  30, 30, 30, 30)
    screen.blit(configEditorBGtxt, (40, SubmenuBGSurfaceXvar + 465))
    BannerFont.set_bold(True)
