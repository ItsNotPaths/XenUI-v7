...
import pygame, sys, math, time, os, shutil, subprocess, textwrap, ast, toml
...
def setup_pygame(): # we draw everything to screen then draw screen onto window, which is the final scaled display surface
    ...
    keycodes = open(join(UIRoot, 'txt', 'pygame keycodes.txt'), 'r')
    keycodes = keycodes.readlines()
    keynames = open(join(UIRoot, 'txt', 'pygame keynames.txt'), 'r')
...
def load_assets():
    ...
    ImportantConfigNames = open(join(UIRoot, 'txt', 'important configs.txt')).readlines() # load this here so we can append the tooltip button surfaces
    ImportantConfigs = []
    txtversion = open(join(Root, 'xenia-canary.config.toml')).readlines()
    counter = 0
    ...
    for i, v in enumerate(ImportantConfigNames):
        for i2, v2 in enumerate(txtversion):
            if v2[:v2.find(' =')] == v.strip():
                ...
                break
    join(UIRoot, 'textures and sfx', 'close.png')
    ui_objects.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'close.png')).convert_alpha(), (80, 80)), 24, 980, 60, 60, 'close'])
    ui_objects.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'credits.png')).convert_alpha(), (80, 80)), 24, 880, 60, 60, 'credits'])
    ui_objects.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'settings.png')).convert_alpha(), (80, 80)), 24, 780, 60, 60, 'settings'])
    ui_objects.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'patch.png')).convert_alpha(), (80, 80)), 24, 680, 60, 60, 'patches'])
    for i, v in enumerate(games):
        try:
            img = pygame.image.load(gamesroot + '\\' + v[0] + '\\' + v[1])
        except:
            img = pygame.image.load(join(UIRoot, 'textures and sfx', 'default banner.png'))
        imgW = img.get_width()
        imgH = img.get_height()
        if imgW > imgH:
            ratio = imgW / imgH
    ...
    for i, v in enumerate(games):
        GameButtons.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'play.png')).convert_alpha(), (banner_height / 1.7, banner_height / 1.7)), 1750, 38 + ((banner_height * 1.3) * i), 60, 60, v[0]])
        ConfigButtons.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'config.png')).convert_alpha(), (banner_height / 1.7, banner_height / 1.7)), 1640, 38 + ((banner_height * 1.3) * i), 60, 60, v[0]])
        buildpaths.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'build path.png')).convert_alpha(), (banner_height / 1.7, banner_height / 1.7)), 1530, 38 + ((banner_height * 1.3) * i), 60, 60, v[0]])
        FilesButtons.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'files.png')).convert_alpha(), (banner_height / 1.7, banner_height / 1.7)), 1420, 38 + ((banner_height * 1.3) * i), 60, 60, v[0]])
    for i, v in enumerate(ImportantConfigs):
        ImportantConfigs[i] = int(v) - 1
        tooltips.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'tooltip.png')).convert_alpha(), (90, 90)), 0, 0, v])
    for i, v in enumerate(settings):
        SettingsTips.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'tooltip.png')).convert_alpha(), (90, 90)), 0, 0, v[v.find('#') + 2:]])
    pygame.font.init()
    ...
    CloseSurface = [pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'close.png')).convert_alpha(), (80, 80)), 220, 0,]
    SaveSurface = [pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'save build path.png')).convert_alpha(), (80, 80)), 320, 0,]
    PasteSurface = [pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'paste.png')).convert_alpha(), (80, 80)), 420, 0,]
    EraseSurface = [pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'erase.png')).convert_alpha(), (80, 80)), 520, 0,]
...
def load_settings():
    ...
    for i, v in enumerate(settings[:-2]):
        ...
        if line == 'True':
            SettingsButtons.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'true.png')).convert_alpha(), (80, 80)), x, (i * 100) - 20, 'bool', True])
        elif line == 'False':
            SettingsButtons.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'false.png')).convert_alpha(), (80, 80)), x, (i * 100) - 20, 'bool', False])
        else:
...
def render():
    ...
    screen.blit(pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'bg.png')).convert_alpha(), (screen_width, screen_height)), (0, 0))
    screen.blit(pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'sidebar.png')).convert_alpha(), (140, 1080)), (0, 0))
    ...
    screen.set_clip(0, 40, screen_width, 900 + SubmenuBGSurfaceYvar)
        ...
        if PasteSurface[0].get_rect().collidepoint((mx - PasteSurface[1], my - PasteSurface[2])):
            ...
            if mousestate == 1:
                try:
                    ...
                    gamesroot = repr(gamesroot).strip("'")
                except:
                    state = -1
...
if getattr(sys, 'frozen', False):
    ...
    UIRoot = os.path.dirname(sys.executable)
else:
    # running as script
    UIRoot = os.path.dirname(os.path.abspath(__file__))
...
settings = open(join(UIRoot, 'txt', 'settings.txt'), 'r')
settings = settings.readlines()
if len(settings) < 6:
    settings.append('')
if not os.path.exists(settings[5].strip()):
    if os.path.exists(join(Root, 'Games')):
        settings.append(join(Root, 'Games'))
    elif os.path.exists(join(Root, 'games')):
        settings.append(join(Root, 'Games'))
    else:
        GamesPopup = True
        while GamesPopup:
            askforgamepath()
        settings[5] = gamesroot
else:
    print(os.path.exists(settings[5].strip()))
    if os.path.exists(join(Root, 'Games')):
        settings[5] = join(Root, 'Games')
if settings[5] != '\\':
...
with open(join (UIRoot, 'txt', 'settings.txt'), 'w+') as i:
    i.write('\n'.join(settings))
    i.close()

icon = pygame.image.load(join(UIRoot, 'textures and sfx', 'canary.png'))
load_settings()
load_config()
load_games()
load_assets()
...
if playintro == True:
    mixer.init()
    mixer.music.load(join(UIRoot, 'textures and sfx', 'startup.wav'))
    mixer.music.set_volume(0.1)
    screen.fill('grey3')
    intro_objects.append([pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'intro bg.png')).convert_alpha(), (MonitorInfo[0], MonitorInfo[1])), (0, 0)])
    intro_objects.append([pygame.image.load(join(UIRoot, 'textures and sfx', 'xenia back.png')).convert_alpha(), (screen_width / 2 - 120, screen_height / 2 - 100)])
    intro_objects.append([pygame.image.load(join(UIRoot, 'textures and sfx', 'X.png')).convert_alpha(), ((screen_width / 2) - 60, (screen_height / 2) - 100)])
    # starting animation
    for i in range (37):
        time.sleep(1 / 60)
...
while run: # this is the main loop. father forgive me for i have sinned
            ...
            if typing != -1:
                ...
                for i, v in enumerate(ImportantConfigs):
                    ...
                    if ConfigSettingsButtons[i][3] == 'text':
                        ...
                        txtversion[v] = txtversion[v].replace(ConfigToSet, str(ConfigValueToSet).strip().lower() + str([' ' * 20])[2:-2])
                    updatedTOMl = toml.dumps(TOMLToEdit) # TOMLToEdit is the doc
                    ...
                    if v2[0] == GameButtons[i][5]:
                        ...
                        for i3, v3 in enumerate(buildpathsdata):
                            if v3[0] == games[i][0]:
                                exepath = v3[1]
                        if exepath == '':
                            exepath = join(Root, 'xenia_canary.exe')
                        gamepath = gamesroot + games[i2][0] + '\\'
                        # raise ValueError(UIRoot, Root)
                        ...
                        if len(files) == 1: # its an iso
                            gamepath += games[i2][0] + '\\' + files[0]
                        else:
                            for i in files:
                                if '.xex' in i: # its a xex
                                    gamepath += games[i2][0] + '\\' + i
                                    break
                        print(join(Root, 'xenia_canary.exe'))
                        try:
                            subprocess.Popen([exepath, gamepath])
                        except:
                            raise ValueError(exepath, gamepath)
                        if CloseLaunch == True:
                            pygame.quit(), exit()
        else:
                ...
                if mousestate == 1: 
                    if v[3] == 'bool':
                        if v[4] == True:
                            ...
                            v[0] = pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'false.png')).convert_alpha(), (80, 80))
                            ConfigSettingsButtons[i][0].set_alpha(140)
                        elif v[4] == False:
                            ConfigSettingsButtons[i][4] = True
                            v[0] = pygame.transform.scale(pygame.image.load(join(UIRoot, 'textures and sfx', 'true.png')).convert_alpha(), (80, 80))
                            ConfigSettingsButtons[i][0].set_alpha(140)
                    else:
                        if v[3] == 'text':
                            typing = i
    ...
    if SaveSurface[0].get_rect().collidepoint((mx - SaveSurface[1], my - SaveSurface[2] )):
        ...
        if mousestate == 1:
            print('saving')
            Settingstxt = open(join(UIRoot, 'txt', 'settings.txt'), 'r').readlines()
            if BuildPathsText != '':
